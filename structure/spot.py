...
class Invertor:
    def __get_from_db(self):
            ...
            self.__last_battery_treshold= invertor_from_db.last_battery_treshold
            self.__is_offline           = (invertor_from_db.is_offline == 1)

    def __init__(self, in_chat_id: str):
        self.__session              = SessionMain()
        self.__chat_id              = in_chat_id
        self.__battery_lvl          = float('99.9')
        self.__last_battery_treshold= float('90.0')
        self.__is_offline           = False
        invertor_from_db = self .__session.query(models.SpotInvertor).filter_by(chat_id=self.__chat_id).first()
    ...
    @property
    def last_battery_treshold(self):
        self.__get_from_db()
        return float(self.__last_battery_treshold) if self.__last_battery_treshold else None
    ...
    @last_battery_treshold.setter
    def last_battery_treshold(self, value: float):
        invertor_for_update = self.__get_invertor_for_update()
        if not invertor_for_update.last_battery_treshold == value:
            invertor_for_update.last_battery_treshold = value
            self.__session.commit()

    @is_offline.setter
    def is_offline(self, value: bool):
        invertor_for_update = self.__get_invertor_for_update()
        if not invertor_for_update.is_offline == (1 if value else 0):
            invertor_for_update.is_offline = (1 if value else 0)
            self.__session.commit()